...
<body>
<form action="rdcUpdate">
	<table>
	<% RDCBean rdcBean = (RDCBean)request.getAttribute("rdcBean");
	if(rdcBean!=null) {
	%>
	<input type="hidden" name="rdcId" value="<%=rdcBean.getRDC_id()%>">
	<tr>
		...
		<td><select name="drpdnCousins">
			...
			<% List<CousinBean> cousins = new CousinDAO().list();
				for(int i=0;i<cousins.size();i++){
					...
					if(rdcBean.getCousin_id()==cousinBean.getCousin_id()){
			%>
			<option value="<%=cousinBean.getCousin_id()%>" selected="selected"><%=cousinBean.getCousin_name()%></option>
			<%}else{ %>
