...
public class RestaurantPhotoInsertServlet extends HttpServlet {
		...
		try {
			...
			for (int i = 0; i < items.size(); i++) {
				...
				if (item.isFormField()) {
					...
					String appPath = context.getRealPath("\\")+("upload\\hotel");
					...
					try {
						...
						if (fileName.isEmpty()) {
							...
							if (context.getMimeType(fileName).equals(
								...
								if(new RestaurantPhotoDAO().insert(restaurantPhotoBean)){
							...
							request.getRequestDispatcher("RestaurantPhoto.jsp").forward(request, response);
							}
						}
					} catch (Exception e) {
						e.printStackTrace();
