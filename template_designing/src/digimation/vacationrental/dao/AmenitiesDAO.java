...
public class AmenitiesDAO {
		...
		public boolean insert(AmenitiesBean amenitiesBean){
			...
			if(conn!=null){
				...
				String insertQuery = "INSERT INTO amenities (amenities_name,property_id) values(?,?)";
				
				try{
					pstmt = conn.prepareStatement(insertQuery);
					
					pstmt.setString(1,amenitiesBean.getAmenitiesName());
					pstmt.setInt(2, amenitiesBean.getProperty_id());
...
public List<AmenitiesBean> Amenitieslist(String propertyId){
			...
			if(conn!=null){
					...
					String selectSQL = "SELECT *from amenities where property_id=?";
					try{
						...
						while(rs.next()){
							...
							amenitiesBean = new AmenitiesBean();
							amenitiesBean.setProperty_id(rs.getInt("property_id"));
							amenitiesBean.setAmenitiesId(rs.getInt("amenities_id"));
							amenitiesBean.setAmenitiesName(rs.getString("amenities_name"));
							listOfAmenities.add(amenitiesBean);
						}
