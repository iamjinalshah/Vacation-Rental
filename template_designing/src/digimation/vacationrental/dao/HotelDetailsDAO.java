...
public class HotelDetailsDAO {
	...
	public boolean insert(HotelDetailsBean hotelDetailsBean) {
		 ...
		 if(conn!=null){
			 String insertSQL = "INSERT INTO HOTEL_DETAILS(hotel_name,address,description,manager_name,email_id,mobile,landline,website,isavailable,area_id,city_id,hrcategory_id,htype_id) values(?,?,?,?,?,?,?,?,?,?,?,?,?)";
	...
	public boolean update(HotelDetailsBean hotelDetailsBean) {
		 ...
		 if(conn!=null){
			 String insertSQL = "UPDATE HOTEL_DETAILS SET hotel_name=?,address=?,description=?,manager_name=?,email_id=?,mobile=?,landline=?,website=?,area_id=?,city_id=?,hrcategory_id=?,htype_id=? WHERE hotel_id=?";
			 ...
			 try {
				...
				pstmt.setInt(13, hotelDetailsBean.getHotel_id());
	...
	public List<HotelDetailsBean> Searchlist(String search){
		...
		if(conn!=null){
			String sql = "SELECT * FROM HOTEL_DETAILS inner join AREA on HOTEL_DETAILS.area_id=AREA.area_id join CITY on HOTEL_DETAILS.city_id=CITY.city_id join STATE on HOTEL_DETAILS.state_id=STATE.state_id join HOTEL_RESTAURANT_CATEGORIES on HOTEL_RESTAURANT_CATEGORIES.hrcategory_id=HOTEL_DETAILS.hrcategory_id where city.city_name=? or area.area_name=? or state.state_name=? and isavailable=?";
			try {
				pstmt = conn.prepareStatement(sql);
				...
				while(rs.next()){
					...
					int hotel_id = rs.getInt("hotel_id");
						...
						String sql1 = "SELECT * FROM HOTEL_PHOTOS WHERE hotel_id=? and iscoverphoto=?";
						...
						if(rs1.next()){
							...
							hotelDetailsBean.setHphoto_name(rs1.getString("photo_name"));
						}
					...
					hotelDetailsBean.setManager_name(rs.getString("manager_name"));
					hotelDetailsBean.setEmail_id(rs.getString("email_id"));
					hotelDetailsBean.setMobile(rs.getString("mobile"));
					hotelDetailsBean.setLandline(rs.getString("landline"));
					hotelDetailsBean.setWebsite(rs.getString("website"));
					hotelDetailsBean.setIsavailable(rs.getBoolean("isavailable"));
					hotelDetailsBean.setArea_name(rs.getString("area_name"));
					hotelDetailsBean.setCity_name(rs.getString("city_name"));
		...
		System.out.println("size:"+listOfHotel.size());
		return listOfHotel;
			
	}
	
	public List<HotelDetailsBean> AllSearchlist(){
		List<HotelDetailsBean> listOfHotel = new ArrayList<HotelDetailsBean>();
		conn = DBConnection.getConnection();
		
		if(conn!=null){
			String sql = "SELECT * FROM HOTEL_DETAILS inner join AREA on HOTEL_DETAILS.area_id=AREA.area_id join CITY on HOTEL_DETAILS.city_id=CITY.city_id join STATE on HOTEL_DETAILS.state_id=STATE.state_id join HOTEL_RESTAURANT_CATEGORIES on HOTEL_RESTAURANT_CATEGORIES.hrcategory_id=HOTEL_DETAILS.hrcategory_id where isavailable=?";
			try {
				pstmt = conn.prepareStatement(sql);
				pstmt.setInt(1, 1);
				rs= pstmt.executeQuery();
				...
				while(rs.next()){
					...
					hotelDetailsBean.setCity_name(rs.getString("city_name"));
					hotelDetailsBean.setHrcategory_name(rs.getString("hrcategory_name"));
					hotelDetailsBean.setCharge(rs.getDouble("charge"));
	...
	public HotelDetailsBean hotelFullInfo(String hotelId){
		HotelDetailsBean hotelDetailsBean = new HotelDetailsBean();
		...
		if(conn!=null){
			...
			String query = "SELECT * FROM HOTEL_DETAILS inner join AREA on HOTEL_DETAILS.area_id=AREA.area_id join CITY on HOTEL_DETAILS.city_id=CITY.city_id join STATE on HOTEL_DETAILS.state_id=STATE.state_id join HOTEL_RESTAURANT_CATEGORIES on HOTEL_RESTAURANT_CATEGORIES.hrcategory_id=HOTEL_DETAILS.hrcategory_id WHERE hotel_id=?";
			try {
				pstmt = conn.prepareStatement(query);
				pstmt.setString(1, hotelId);
				rs=pstmt.executeQuery();
	...
	public List<HotelDetailsBean> photolist(){
		List<HotelDetailsBean> listOfHotel = new ArrayList<HotelDetailsBean>();
		conn = DBConnection.getConnection();
		
		if(conn!=null){
			String sql = "SELECT * FROM HOTEL_PHOTOS WHERE iscoverphoto=? ";
			try {
				pstmt = conn.prepareStatement(sql);
				pstmt.setInt(1, 0);
				rs= pstmt.executeQuery();
				...
				while(rs.next()){
					...
					hotelDetailsBean.setHotel_id(rs.getInt("hotel_id"));
					hotelDetailsBean.setHphoto_name(rs.getString("photo_name"));
					listOfHotel.add(hotelDetailsBean);
