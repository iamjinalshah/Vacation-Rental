...
public class CarDetailsDAO {
	...
	public boolean insert(CarDetailsBean carDetailsBean) {
		 ...
		 if(conn!=null){
			 String insertSQL = "INSERT INTO CAR_DETAILS(capacity,manager_name,email_id,mobile,landline,isavailable,area_id,city_id,carbrand_id,carmodel_id) values(?,?,?,?,?,?,?,?,?,?)";
	...
	public List<CarDetailsBean> Searchlist(String search){
		List<CarDetailsBean> listOfcar = new ArrayList<CarDetailsBean>();
		conn = DBConnection.getConnection();
		
		if(conn!=null){
			String sql = "SELECT * FROM CAR_DETAILS inner join AREA on CAR_DETAILS.area_id=AREA.area_id join CITY on CAR_DETAILS.city_id=CITY.city_id join STATE on CAR_DETAILS.state_id=STATE.state_id join CAR_BRANDS on CAR_BRANDS.carbrand_id=CAR_DETAILS.carbrand_id join CAR_MODELS on CAR_DETAILS.carmodel_id=CAR_MODELS.carmodel_id where city.city_name=? or area.area_name=? or state.state_name=? and isavailable=?";
			try {
				pstmt = conn.prepareStatement(sql);
	...
	public List<CarDetailsBean> AllSearchlist(){
		List<CarDetailsBean> listOfcar = new ArrayList<CarDetailsBean>();
		conn = DBConnection.getConnection();
		
		if(conn!=null){
			String sql = "SELECT * FROM CAR_DETAILS inner join AREA on CAR_DETAILS.area_id=AREA.area_id join CITY on CAR_DETAILS.city_id=CITY.city_id join STATE on CAR_DETAILS.state_id=STATE.state_id join CAR_BRANDS on CAR_BRANDS.carbrand_id=CAR_DETAILS.carbrand_id join CAR_MODELS on CAR_DETAILS.carmodel_id=CAR_MODELS.carmodel_id where isavailable=?";
			try {
				pstmt = conn.prepareStatement(sql);
				pstmt.setInt(1, 1);
				rs= pstmt.executeQuery();
