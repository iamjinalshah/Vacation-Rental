...
public class PropertyPhotoDAO {
	...
	public boolean insert(PropertyPhotoBean propertyPhotoBean) {
		 ...
		 if(conn!=null){
			 ...
			 try {
				...
				pstmt.setInt(2, 1);
				pstmt.setInt(3, 0);
				pstmt.setString(4, propertyPhotoBean.getProperty_id());
	...
	public List<PropertyPhotoBean> photolist(String propertyId){
		List<PropertyPhotoBean> photo = new ArrayList<PropertyPhotoBean>();
		conn = DBConnection.getConnection();
		
		if(conn!=null){
			String sql = "SELECT * FROM PROPERTY_PHOTOS where property_id=?";
			try {
				pstmt = conn.prepareStatement(sql);
				...
				while(rs.next()){
					...
					propertyPhotoBean.setIsvisible(rs.getBoolean("isvisible"));
					propertyPhotoBean.setIscoverpage(rs.getBoolean("iscoverphoto"));
					propertyPhotoBean.setProperty_id(rs.getString("property_id"));
	...
	public boolean update(PropertyPhotoBean propertyPhotoBean) {
		 ...
		 if(conn!=null){
			 ...
			 try {
				...
				pstmt.setString(1,propertyPhotoBean.getPhoto_name());
				pstmt.setString(2, propertyPhotoBean.getProperty_id());
	...
	public List<PropertyPhotoBean> propertyPhotolist(String propertyId){
		List<PropertyPhotoBean> listOfPropertyPhotos = new ArrayList<PropertyPhotoBean>();
		conn = DBConnection.getConnection();
		
		if(conn!=null){
			String sql = "SELECT * FROM PROPERTY_PHOTOS WHERE property_id=? AND iscoverphoto=? ";
			try {
				pstmt = conn.prepareStatement(sql);
				pstmt.setString(1, propertyId);
				pstmt.setInt(2, 0);
				...
				while(rs.next()){
					...
					listOfPropertyPhotos.add(propertyPhotoBean);
