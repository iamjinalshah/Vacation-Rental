...
public class RestaurantPhotoDAO {
	...
	public List<RestaurantPhotoBean> restaurantPhotolist(String restaurantId){
		List<RestaurantPhotoBean> listOfRestaurantPhotos = new ArrayList<RestaurantPhotoBean>();
		conn = DBConnection.getConnection();
		
		if(conn!=null){
			String sql = "SELECT * FROM RESTAURANT_PHOTOS WHERE restaurant_id=? AND iscoverphoto=? ";
			try {
				pstmt = conn.prepareStatement(sql);
				...
				while(rs.next()){
					...
					listOfRestaurantPhotos.add(restaurantPhotoBean);
