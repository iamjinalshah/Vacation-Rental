...
public class HotelPhotoDAO {
	...
	public List<HotelPhotoBean> Hotelphotolist(String hotelId){
		List<HotelPhotoBean> listOfHotelPhotos = new ArrayList<HotelPhotoBean>();
		conn = DBConnection.getConnection();
		
		if(conn!=null){
			String sql = "SELECT * FROM HOTEL_PHOTOS WHERE hotel_id=? AND iscoverphoto=? ";
			try {
				pstmt = conn.prepareStatement(sql);
				...
				while(rs.next()){
					...
					listOfHotelPhotos.add(hotelPhotoBean);
