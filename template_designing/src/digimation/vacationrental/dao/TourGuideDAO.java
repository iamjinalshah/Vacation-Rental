...
public class TourGuideDAO {
	...
	public boolean insert(TourGuideBean tourGuideBean) {
		 ...
		 if(conn!=null){
			 String insertSQL = "INSERT INTO TOUR_GUIDE(first_name,middle_name,last_name,mobile,landline,photo,rate,area_id,city_id) values(?,?,?,?,?,?,?,?,?)";
	...
	public List<TourGuideBean> Searchlist(String search){
		...
		if(conn!=null){
			String sql = "SELECT * FROM TOUR_GUIDE inner join AREA on TOUR_GUIDE.area_id=AREA.area_id join CITY on TOUR_GUIDE.city_id=CITY.city_id join STATE on TOUR_GUIDE.state_id=STATE.state_id where city.city_name=? or area.area_name=? or state.state_name=? ";
			try {
				pstmt = conn.prepareStatement(sql);
				...
				while(rs.next()){
					...
					tourGuideBean.setMiddle_name(rs.getString("middle_name"));
					tourGuideBean.setLast_name(rs.getString("last_name"));
					tourGuideBean.setMobile(rs.getString("mobile"));
					tourGuideBean.setLandline(rs.getString("landline"));
					tourGuideBean.setPhoto(rs.getString("photo"));
					tourGuideBean.setRate(rs.getInt("rate"));
					tourGuideBean.setArea_id(rs.getInt("area_id"));
					tourGuideBean.setCity_id(rs.getInt("city_id"));
					tourGuideBean.setCity_name(rs.getString("city_name"));
	...
	public List<TourGuideBean> AllSearchlist(){
		List<TourGuideBean> listOfGuide = new ArrayList<TourGuideBean>();
		conn = DBConnection.getConnection();
		
		if(conn!=null){
			String sql = "SELECT * FROM TOUR_GUIDE inner join AREA on TOUR_GUIDE.area_id=AREA.area_id join CITY on TOUR_GUIDE.city_id=CITY.city_id join STATE on TOUR_GUIDE.state_id=STATE.state_id";
			try {
				pstmt = conn.prepareStatement(sql);
				rs= pstmt.executeQuery();
	...
	public TourGuideBean tourguideFullInfo(String tourguideId){
		TourGuideBean tourGuideBean = new TourGuideBean();
		conn = DBConnection.getConnection();
		
		if(conn!=null){
			String sql = "SELECT * FROM TOUR_GUIDE inner join AREA on TOUR_GUIDE.area_id=AREA.area_id join CITY on TOUR_GUIDE.city_id=CITY.city_id join STATE on TOUR_GUIDE.state_id=STATE.state_id where tourguide_id=?";
			try {
				pstmt = conn.prepareStatement(sql);
				pstmt.setString(1, tourguideId);
				rs= pstmt.executeQuery();
				...
				while(rs.next()){
					...
					tourGuideBean.setCity_id(rs.getInt("city_id"));
					tourGuideBean.setCity_name(rs.getString("city_name"));
					tourGuideBean.setState_name(rs.getString("state_name"));
